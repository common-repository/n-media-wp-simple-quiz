...
function wqt_admin_add_columns_data($column, $question_id) {
    ...
    if( class_exists('NM_MCQS_PRO') ) {
        $allow_time     = $question_class->get_option('wqt_allow_time', 'NO');
        
        $allow_result   = $question_class->get_option('wqt_allow_result', 'NO');
    ...
    switch ( $column ) {
        ...
        case 'quiz_id' :
              echo '[wqt-question id="'.esc_attr($question_id).'"]';
        break;

        case 'quiz_question':
              echo esc_attr($question_count);
        break;
        case 'quiz_marks':
              echo esc_attr($total_marks);
        break;
        case 'ans_select':
              echo strtoupper(esc_attr($ans_selection));
        break;
        case 'enable_time':
              echo strtoupper(esc_attr($allow_time));
        break;
        case 'show_result':
              echo strtoupper(esc_attr($allow_result));
        break;
    }
}
...
function wqt_result_admin_add_columns_data($column, $question_id) {
    ...
    switch ( $column ) {
        ...
        case 'result_stat':
            $status = "{$obtain_marks}/{$total_marks} ({$percenatge})";
            echo esc_attr($status);
        break;
        case 'wqt_time':
              echo esc_attr($consume_time);
        break;
        case 'correct_ques':
             echo esc_attr($question_class->wqt_get_correct_question());
        break;
        case 'user_info':   
            echo esc_attr(get_user_info($question_id));
        break;
        case 'form_name':   
